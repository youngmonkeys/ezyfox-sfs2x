 ...
 *
 */
public class SfsVariableTransformerTest extends BaseCommandTest {
    
    public SfsVariableTransformerTest() {
        super();
        init();
    }
    ...
    public void test() {
        ...
        SfsVariableTransformer variableTransformer = new SfsVariableTransformer();
        List<Variable> vars = new ArrayList<>();
        vars.add(new SFSUserVariable("1", null));
        vars.add(new SFSUserVariable("2", true));
        vars.add(new SFSUserVariable("3", new Double(1D)));
        vars.add(new SFSUserVariable("4", new Integer(2)));
        vars.add(new SFSUserVariable("5", new String("3")));
        vars.add(new SFSUserVariable("6", value));
        vars.add(new SFSUserVariable("7", value.getSFSArray("array")));
        Parameters newVars = variableTransformer.transform(vars);
        assertEquals(newVars.get("1"), null);
        assertEquals(newVars.get("2"), Boolean.TRUE);
        assertEquals(newVars.get("3"), new Double(1D));
